  ...
  lessons
    ...
    .forEach(lesson => {
      lesson.menu_items.forEach(menuItem => {
        const menuName = category === "food" ? menuItem.name : menuItem.product_name;
...
export const MyLessons = () => {
  ...
  const groupedFoodUnits = groupLessonsByUnit(lessons, 'food');
  ...
  const getTabContent = (type) => {
    ...
    // const groupedWineUnits = groupLessonsByMenu(lessons, 'wine');
  ...
  function handleStartQuiz(type, unit, chapter,) {
    ...
    if (foundLesson) {
      ...
      const attemptedIds = (foundLesson.attemptedQuestions || []).map(aq => aq.questionId || aq.uuid);
