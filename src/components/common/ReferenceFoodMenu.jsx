...
const ReferenceFoodMenuPanel = ({ isOpen, onClose, menuData }) => {
  ...
  const menuSections = React.useMemo(() => {
    ...
    return menuData.reduce((acc, dish) => {
      ...
      const allergens = Array.isArray(dish.allergens) ? dish.allergens.join(', ') : (dish.allergens || 'None');
      // Health restrictions: flatten if object, else use as string/array
      let healthRestrictions = 'None';
      if (dish.dietary_restrictions) {
        if (typeof dish.dietary_restrictions === 'object' && dish.dietary_restrictions.health) {
          healthRestrictions = Array.isArray(dish.dietary_restrictions.health) ? dish.dietary_restrictions.health.join(', ') : dish.dietary_restrictions.health;
      ...
      if (dish.dietary_restrictions && typeof dish.dietary_restrictions === 'object' && dish.dietary_restrictions.belief) {
        beliefAccommodations = Array.isArray(dish.dietary_restrictions.belief) ? dish.dietary_restrictions.belief.join(', ') : dish.dietary_restrictions.belief;
      }
      // Lifestyle accommodations
      let lifestyleAccommodations = 'None';
      if (dish.dietary_restrictions && typeof dish.dietary_restrictions === 'object' && dish.dietary_restrictions.lifestyle) {
        lifestyleAccommodations = Array.isArray(dish.dietary_restrictions.lifestyle) ? dish.dietary_restrictions.lifestyle.join(', ') : dish.dietary_restrictions.lifestyle;
      }
      // Substitution accommodations
      let substitutionAccommodations = 'None';
      if (dish.substitution_accommodations) {
        substitutionAccommodations = Array.isArray(dish.substitution_accommodations) ? dish.substitution_accommodations.join(', ') : dish.substitution_accommodations;
      }
      // Substitution notes
          ...
          <div key={section.section}>
            ...
            <div className="space-y-4">
              {section.items.map(item => (
                <div key={item.name} className="w-full bg-background rounded-lg px-3 sm:px-4 py-3 flex flex-col">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-black text-base sm:text-base md:text-lg text-left">{item.name}</span>
                    <span className="text-black text-sm font-semibold text-right">{item.price}</span>
                  </div>
                  <div className="text-gray-700 text-sm mb-1 text-left">{item.description}</div>
                  <div className="text-sm text-left"><span className="font-bold">Allergens Included: </span>{item.allergens}</div>
                  <div className="text-sm text-left"><span className="font-bold">Health Restrictions Included: </span>{item.healthRestrictions}</div>
                  <div className="text-sm text-left"><span className="font-bold">Belief Accommodations: </span>{item.beliefAccommodations}</div>
                  <div className="text-sm text-left"><span className="font-bold">Lifestyle Accommodations: </span>{item.lifestyleAccommodations}</div>
                  <div className="text-sm text-left"><span className="font-bold">Substitution Accommodations: </span>{item.substitutionAccommodations}</div>
                  <div className="text-sm text-left"><span className="font-bold">Substitution Notes: </span>{item.substitutionNotes}</div>
                </div>
              ))}
            </div>
          </div>
